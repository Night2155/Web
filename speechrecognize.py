...
def get_large_audio_transcription(path, filename):
    ...
    folder_name = f"D:\\audio_save\\writing\\audio-chunk"
    ...
    if not os.path.isdir(folder_name):
        ...
        os.mkdir(folder_name)
    f = open(fr'D:\audio_save\writing\text\{filename}.txt', "w", encoding='UTF-8')
    # process each chunk
...
for info in os.listdir(r'D:\audio_save\writing\wav'):
    domain = os.path.abspath(r'D:\audio_save\writing\wav') #獲取資料夾的路徑，此處其實沒必要這麼寫，目的是為了熟悉os的資料夾操作
